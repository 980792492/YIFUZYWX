    ...
    <ul class="ul-list">
      <li v-for='item in items' class="report-list">
        ...
        <div class="rank">
          ...
          <div class="empty"></div>
          <div class="rank2">
            <p class="p1" :class="item.rate >= 0 ? 'red' : 'blue'  ">{{item.rate | toNumber }}%</p>
            <p class="p2">当日收益</p>
          </div>
          <div class="empty"></div>
          <div class="rank3">
...
//    }
      this.pullStatus = 1
      this.api.get('Rank/UserInfo', params).then(resp => {
...
<style scoped>
  ...
  .report-list .timer { border-bottom: 0.01rem dashed #e8cfb1; line-height: 0.53rem; color: #b3804b; font-size: 0.2rem;}
  ...
  .rank div .p1 { color: #e22a2a; font-size: 0.26rem; padding-top: 0.19rem;}
  .rank div .p2 { color: #c6a989; font-size: 0.2rem;}
  .info { padding-top: 0.06rem; overflow: hidden;}
  .info .info1 {line-height: 0.4rem; color: #b27f4a;}
  .info .info1 span { margin-right: 0.15rem;}
  .info .info2 { overflow: hidden; display: flex; padding-top: 0.11rem; font-size: 0.18rem; color: #b27f4a; padding-bottom: 0.08rem;}
